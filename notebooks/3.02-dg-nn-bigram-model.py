# ---
# jupyter:
#   jupytext:
#     text_representation:
#       extension: .py
#       format_name: percent
#       format_version: '1.3'
#       jupytext_version: 1.19.1
#   kernelspec:
#     display_name: Python 3 (ipykernel)
#     language: python
#     name: python3
# ---

# %%
import sys

sys.path.append("..")

# %%
import os
from pathlib import Path

import numpy as np
import pandas as pd
import matplotlib.pyplot as plt

import torch
from torch import nn
import torch.nn.functional as F

from namegen.dataset import uk_towns_and_counties
from namegen.modeling.model import BigramsModel, OneLayerBigramModel
from namegen.modeling.train import train_bigram_model, Trainer, Hyper, Result
from namegen.modeling.predict import generate, calculate_loss


# %%
dataset = uk_towns_and_counties('../data')

# %%
baseline_model = train_bigram_model(dataset, 0.1)
random_model = BigramsModel(torch.ones((dataset.nalphabet, dataset.nalphabet)), nalphabet=dataset.nalphabet)

# %%
optimizer = torch.optim.AdamW
hyper = [
    Hyper(name='300 steps', context_size=1, model=OneLayerBigramModel, batch_size=100, nsteps=300, lr=1e-3, optimizer=optimizer),
    Hyper(name='1000 steps', context_size=1, model=OneLayerBigramModel, batch_size=100, nsteps=1000, lr=1e-3, optimizer=optimizer),
    Hyper(name='3000 steps', context_size=1, model=OneLayerBigramModel, batch_size=100, nsteps=3000, lr=1e-3, optimizer=optimizer),
    Hyper(name='10000 steps', context_size=1, model=OneLayerBigramModel, batch_size=100, nsteps=10000, lr=1e-3, optimizer=optimizer),
]

# %%
trainer = Trainer(dataset)

# %%
results = []
for h in hyper:
    results.append(trainer.run_scenario(h))

# %%
for r in results:
    npars = sum(p.numel() for p in r.model.parameters())
    print(f"model={r.hyper.model.__name__} {npars=}")

# %%
fig, ax1 = plt.subplots(1, 1, figsize=(5,  4))

for result in results:
    ax1.plot(result.steps, result.train_metrics['loss'], label=result.hyper.name)
ax1.set_xlabel("Steps")
ax1.set_title("Train accuracy")
ax1.grid(True)
ax1.legend()

plt.plot()

# %%
print(calculate_loss(dataset, random_model))
for r in results:
    print(calculate_loss(dataset, r.model))
print(calculate_loss(dataset, baseline_model))

# %%
seed = 649431853

df = pd.DataFrame()

torch.random.manual_seed(seed)
df['random'] = generate(dataset, random_model)

for r in results:
    torch.random.manual_seed(seed)
    df[r.hyper.name] = generate(dataset, r.model)

torch.random.manual_seed(seed)
df['baseline'] = generate(dataset, baseline_model)

df

# %%
model = results[-1].model

df = pd.DataFrame()
for T in [0.1, 0.5, 0.9, 1.0, 1.1, 1.2, 1.5]:
    name = f"{T:.2}"
    torch.random.manual_seed(seed)
    df[name] = generate(dataset, model, T=T)

df
